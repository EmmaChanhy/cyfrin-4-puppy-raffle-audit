...
// @audit overflow/underflow before 0.8.0
// Overflow/underflow Solution: use safeMath
...
/// Full audit report: https://www.codehawks.com/report/clo383y5c000jjx087qrkbrj8
...
contract PuppyRaffle is ERC721, Ownable {
    ...
    // Overflow Solution: change uint64 to uint256
    ...
    function enterRaffle(address[] memory newPlayers) public payable {
        ...
        // @audit Denial of service due to huge amount of gas
    ...
    // @audit potential front-running, see the result in mempool, if not winner then refund
    // Front-running solution: apply time lock
    function refund(uint256 playerIndex) public {
        ...
        // @audit reentrancy not follow the Check-effects-interactions
        // payable(msg.sender).sendValue(entranceFee);

        // players[playerIndex] = address(0);
        // emit RaffleRefunded(playerAddress);

        // @audit Reentrancy Solution: perform CEI
        // @audit Address(0) Solution: delete player index
    ...
    function selectWinner() external returns (address) {
        ...
        // Solution: Use Chainlink's VRF
        uint256 winnerIndex = uint256(
            keccak256(
                abi.encodePacked(msg.sender, block.timestamp, block.difficulty)
            )
        ) % players.length;
        // @audit winner can be address(0) and lose funds
        ...
        // @audit Casting from uint256 to uint64 may cause overflow or incorrect value
        // Overflow solution: don't cast fee to uint64
        ...
        // @audit Always revert if the winner is address(0)
    ...
    function withdrawFees() external {
        // @audit send ether to the contract and destroy the balance
        // require(
